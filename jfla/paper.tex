...
\begin{tikzpicture}[nodes = {align = center}]
  ...
  \node (fjs) [draw, above right = 0.5cm and 1cm of focaml, text width=2cm] {Interpréteur en fJS};
  \node (fjsa) [draw, below right = 0.5cm and 1cm of focaml, text width=2cm] {Interpréteur annoté en fJS};
  \node (ast) [draw, right = of fjsa, text width=2cm] {AST};
  \node (source) [draw, right = of fjs, text width=2cm, text centered] {Programme};
...
\asi{dire que l'on interprète du code annoté quand on veut l'afficher, et non
  annoté quand on ne veut pas l'afficher.}

Cet interpréteur visuel contient des instructions permettant de générer des 
...
ce qui peut donner du code long et fastidieux.
